...
export default function HistoryAbsensi() {
  ...
  useEffect(() => {
    loadHistory();
    setCurrentPage(1);
  }, [selectedKelas, selectedTanggal, selectedMonth, search]);
  ...
  const handleExport = () => {
    const result = history.filter((item) => {
      const kelasMatch =
        !exportKelas ||
        item.nama_kelas === exportKelas ||
        item.id_kelas === exportKelas;

      const statusDisplay = getDisplayStatus(item);

      const itemStatus = item.status_kehadiran;

      const statusMatch =
        exportStatus === "semua" ||
        itemStatus === exportStatus ||
        statusDisplay.key === exportStatus;
        ...
        <div className="overflow-x-auto bg-white rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-5 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="px-4 py-2 rounded-xl bg-[#1d2433] text-white text-[13px] font-bold">
