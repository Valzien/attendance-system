...
const navItems = [
  { to: '/guru/home',       title: 'Dashboard',       icon: LayoutDashboard },
  { to: '/guru/history',      title: 'History Absensi',   icon: UserCheck       },
  { to: '/guru/pengajuan',  title: 'Data Pengajuan',  icon: ClipboardList   },
  { to: '/guru/data-siswa', title: 'Data Siswa', icon: Contact },
  { to: '/guru/jadwal',     title: 'Jadwal Pelajaran', icon: CalendarDays   },
];
      ...
      <aside
        ...
        <div className="px-5 py-5 flex items-center gap-3 border-b border-gray-100">
          ...
          <div className="ml-1">
            ...
            <p className="text-[10px] text-gray-400 font-medium">Portal Guru</p>
          </div>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[13px] font-${isActive ? 'bold' : 'medium'} flex-1`}>{item.title}</span>
        ...
        <div className="px-4 py-4 border-t border-gray-100">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full bg-[#c7f5d8] flex items-center justify-center text-[11px] font-black text-[#2b8a57] flex-shrink-0">
              {user?.name ? user.name.charAt(0).toUpperCase() : 'G'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-[#1d2433] truncate">{user?.name || 'Guru'}</p>
              <p className="text-[10px] text-gray-400">Guru</p>
            </div>
          </div>
          <button
        ...
        {showHeader && (
          <header className="bg-white border-b border-gray-100 px-5 lg:px-8 py-4 flex items-center justify-between shadow-[0_1px_8px_rgb(0,0,0,0.03)] sticky top-0 z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[15px] font-black text-[#1d2433]">
                  {navItems.find(n => n.to === location.pathname)?.title || 'Portal Guru'}
                </p>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => navigate('/guru/notifications')}
              ...
              >
                <div className="w-6 h-6 rounded-full bg-[#c7f5d8] flex items-center justify-center text-[10px] font-black text-[#2b8a57]">
                  {user?.name ? user.name.charAt(0).toUpperCase() : 'G'}
                </div>
                <span className="text-[12px] font-bold text-gray-700 hidden sm:block">{user?.name || 'Guru'}</span>
              </button>
            </div>
          </header>
        )}
