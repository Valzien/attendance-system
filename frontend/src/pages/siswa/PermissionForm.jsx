...
export default function PermissionForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formPayload.append("nama_siswa", formData.name);

      formPayload.append("kelas", formData.class);

      formPayload.append("tanggal", today);

      formPayload.append("alasan", formData.reason);

      if (selectedFile) {
        formPayload.append("lampiran", selectedFile);
      }

      const res = await fetch("${API_BASE}/izin/submit", {
        method: "POST",
        body: formPayload,
      });
      ...
      if (!result.success) {
        alert(result.message || "Gagal mengirim izin");
            ...
            <div className="flex-1 flex flex-col gap-8">
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex-1 w-full bg-[#fffcf5] border-[3px] border-dashed border-[#fbbc3e] rounded-xl flex flex-col items-center justify-center text-gray-500 hover:bg-[#fff9e6] transition-colors cursor-pointer relative min-h-[160px]">
                  ...
                  <Camera size={36} className="text-[#8c92a6] mb-3" />
